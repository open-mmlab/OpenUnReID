...
# Main interface
cpdef evaluate_cy(distmat, q_pids, g_pids, q_camids, g_camids, max_rank, use_metric_cuhk03=False, use_metric_vehicleid=True):
    ...
    elif use_metric_vehicleid:
        return eval_vehicleid_cy(distmat, q_pids, g_pids, q_camids, g_camids, max_rank)
    else:
...
cpdef eval_vehicleid_cy(float[:,:] distmat, long[:] q_pids, long[:]g_pids,
                     long[:]q_camids, long[:]g_camids, long max_rank):
