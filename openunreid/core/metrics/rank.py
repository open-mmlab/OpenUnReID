...
def eval_vehicleid(distmat, q_pids, g_pids, q_camids, g_camids, max_rank):
    ...
    indices = np.argsort(distmat, axis=1)   # [5693, 800]
    matches = (g_pids[indices] == q_pids[:, np.newaxis]).astype(np.int32)
...
def evaluate_py(
    distmat, q_pids, g_pids, q_camids, g_camids, max_rank, use_metric_cuhk03, use_metric_vehicleid
):
